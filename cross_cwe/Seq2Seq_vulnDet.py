...
train_data = train_data[["processed_func", "flaw_line", "flaw_line_index", "CWE ID"]]
train_data = train_data.reset_index(drop=True)

train_data = pd.DataFrame(({'Text': train_data['processed_func'], 'Lines':train_data['flaw_line'], 'Line_Index':train_data['flaw_line_index'], 'CWE':train_data['CWE ID']}))
...
# keep the useful for Seq2Seq columns
val_data = val_data[["processed_func", "flaw_line", "flaw_line_index", "CWE ID"]]
val_data = val_data.reset_index(drop=True)

val_data = pd.DataFrame(({'Text': val_data['processed_func'], 'Lines':val_data['flaw_line'], 'Line_Index':val_data['flaw_line_index'], 'CWE':val_data['CWE ID']}))
...
# keep the useful for Seq2Seq columns
test_data = test_data[["processed_func", "flaw_line", "flaw_line_index", "CWE ID"]]
test_data = test_data.reset_index(drop=True)

test_data = pd.DataFrame(({'Text': test_data['processed_func'], 'Lines':test_data['flaw_line'], 'Line_Index':test_data['flaw_line_index'], 'CWE':test_data['CWE ID']}))
...
# Single out randomly some CWEs to remove them from training set in order to test cross-type performance

# In[27]:


n_cwes = 5
#random_cwes = pd.Series(train_data['CWE'].dropna().unique()).sample(n_cwes, random_state=seed).tolist()
random_cwes = ['CWE-264', 'CWE-189', 'CWE-399', 'CWE-200', 'CWE-190'] # CWEs with a substantial amount of instances in the test data

print(len(train_data))
train_data = train_data[~train_data["CWE"].isin(random_cwes)]
print(len(train_data))
