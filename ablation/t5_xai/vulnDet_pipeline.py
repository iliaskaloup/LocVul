...
from transformers import AutoTokenizer, RobertaTokenizer, AutoModelForSequenceClassification, T5ForSequenceClassification #, BertModel, BertTokenizer
...
for i, sample in enumerate(positive_samples):
    ...
    if EXPLAINER == "LIME":
        ...
        with torch.no_grad():
            encodings = tokenizer(sample, return_tensors='pt', padding=True, truncation=True, max_length=max_len).to(device)
            outputs = model(**encodings, output_attentions=True, return_dict=True)
            logits = outputs.logits.cpu().detach().numpy()
            #attentions = outputs.attentions  # Attention weights from each layer 
            enc_atts = outputs.cross_attentions # enc_atts = outputs.encoder_attentions  # outputs.cross_attentions

        batch_attention = enc_atts[0][0]
