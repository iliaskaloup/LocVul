...
# define functions
def save_checkpoint(filename, epoch, model, optimizer, scheduler, train_loss_per_epoch, val_loss_per_epoch,
                    train_f1_per_epoch, val_f1_per_epoch):
...
train_data = pd.DataFrame(({'Text': train_data['processed_func'], 'Labels': train_data['target'],
                            'Lines': train_data['flaw_line'], 'Line_Index': train_data['flaw_line_index']}))
...
val_data = pd.DataFrame(({'Text': val_data['processed_func'], 'Labels': val_data['target'],
                          'Lines': val_data['flaw_line'], 'Line_Index': val_data['flaw_line_index']}))
val_data.head()

test_data = test_data[test_data["project"] != "Chrome"]

test_data = pd.DataFrame(({'Text': test_data['processed_func'], 'Labels': test_data['target'],
                           'Lines': test_data['flaw_line'], 'Line_Index': test_data['flaw_line_index']}))
...
if not FINE_TUNE and os.path.exists(save_path):
    ...
    for epoch_num in range(n_epochs):
        ...
        logger.info(
            f"Epoch {epoch_num + 1}/{n_epochs} - Train Loss: {train_loss_per_epoch[-1]:.4f} - Valid Loss: {val_loss_per_epoch[-1]:.4f}")
        ...
        if val_f1 > best_val_f1:
            ...
            save_checkpoint(save_path, epoch_num + 1, model, optimizer.state_dict(), scheduler.state_dict(),
                            train_loss_per_epoch, val_loss_per_epoch, train_f1_per_epoch, val_f1_per_epoch)
            ...
            if no_improvement_counter >= patience:
                ...
                logger.info(
                    f"Early stopping after epoch {total_epochs}. Best epoch: {best_epoch} with best F1 score: {best_val_f1:.4f}")
                break
...
if EXPLAIN_ONLY_TP:
    ...
    true_positive_indices = [i for i, (pred, label) in enumerate(zip(test_pred, Y_test.tolist())) if
                             pred == 1 and label == 1]
    ...
    positive_indices = [i for i, pred in enumerate(test_pred) if
                        pred == 1]  # Indexes of Positive predictions (TPs + FPs)
...
for i, sample in enumerate(positive_samples):
    ...
    if EXPLAINER == "LIME":
        ...
        explanation = [(line_idx + 1, line, line_deepliftshap_scores[line_idx]) for line_idx, line in enumerate(lines)]
        ...
        with torch.no_grad():
            encodings = tokenizer(sample, return_tensors='pt', padding=True, truncation=True, max_length=max_len).to(
                device)
        ...
        explanation = [(line_idx + 1, line, line_attention_scores[line_idx]) for line_idx, line in enumerate(lines)]
...
for idx, explanation in enumerate(explanation_results):
    ...
    if EXPLAINER == "DEEPLIFTSHAP":
        ...
        line_scores = [token_scores_dict.get(line_idx + 1, 0) for line_idx in range(len(lines))]

    # Create a list of tuples containing (line_index, line_text, lime_score)
    line_scores_with_text = [(line_idx + 1, line, line_scores[line_idx]) for line_idx, line in enumerate(lines)]
