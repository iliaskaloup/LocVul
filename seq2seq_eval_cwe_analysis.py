...
# Specific CWE evaluation

# In[9]:


# # if you need to use a specific CWE
# # first, rank the CWE ID occurences

# cwe_eval = True

# if cwe_eval:
#     top_10_cwe = dataset['CWE ID'].value_counts()
#     print(top_10_cwe)
    
#     CWE = top_10_cwe.index.tolist()[0]
#     dataset = dataset[dataset["CWE ID"] == CWE]
...
train_data = train_data[["processed_func", "target", "flaw_line", "flaw_line_index", "CWE ID"]]
...
train_data = pd.DataFrame(({'Text': train_data['processed_func'], 'Labels': train_data['target'], 'Lines':train_data['flaw_line'], 'Line_Index':train_data['flaw_line_index'], 'CWE':train_data['CWE ID']}))
#train_data = train_data[0:100]
train_data.head()

val_data = pd.DataFrame(({'Text': val_data['processed_func'], 'Labels': val_data['target'], 'Lines':val_data['flaw_line'], 'Line_Index':val_data['flaw_line_index'], 'CWE':val_data['CWE ID']}))
val_data.head()

test_data = pd.DataFrame(({'Text': test_data['processed_func'], 'Labels': test_data['target'], 'Lines':test_data['flaw_line'], 'Line_Index':test_data['flaw_line_index'], 'CWE':test_data['CWE ID']}))
...
if cwe_eval:
    top_10_cwe = test_data['CWE'].value_counts().head(10)
    print(top_10_cwe)
...
if cwe_eval:
    CWE = top_10_cwe.index.tolist()[0]
    test_data = test_data[test_data["CWE"] == CWE]
...
only_line_level = False

if only_line_level == False:
    ...
    actual_positive_indices = [i for i, label in enumerate(Y_test.tolist()) if label == 1]  # Indices of Actual Positive predictions (TPs + FNs)
    positive_indices = actual_positive_indices
...
positive_cwes = [test_data['CWE'].tolist()[i] for i in positive_indices]
positive_cwe_counts = dict(Counter(positive_cwes))

ranked_cwe_counts = dict(sorted(positive_cwe_counts.items(), key=lambda x: x[1], reverse=True))

# Print nicely
for cwe, rate in ranked_cwe_counts.items():
    print(f"{cwe}: {rate:.2f}")
...
# Find the hallucinated samples and their CWEs

# In[36]:


hall_cwes = [positive_cwes[i] for i in halls]
cwe_counts = dict(Counter(hall_cwes))
cwe_rates = {
    cwe: cwe_counts[cwe] / positive_cwe_counts[cwe]
    for cwe in cwe_counts
    if cwe in positive_cwe_counts and positive_cwe_counts[cwe] > 0
}
cwe_rates
...
ranked_cwe_rates = dict(sorted(cwe_rates.items(), key=lambda x: x[1], reverse=True))

# Print nicely
for cwe, rate in ranked_cwe_rates.items():
    print(f"{cwe}: {rate:.2f}")
